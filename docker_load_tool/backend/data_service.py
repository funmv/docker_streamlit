"""
데이터 처리 서비스 (Backend Logic)
FastAPI 스타일로 구조화된 로직
"""
import pandas as pd
import logging
import re
...
class DataService:
    ...
    def load_data(
        ...
        if file_type == 'excel':
            return self._load_excel(file_obj, config, sheet_name, progress_callback)
        else:
            return self._load_csv(file_obj, config)
    
    def _load_csv(self, file_obj, config: Dict) -> pd.DataFrame:
        """CSV 파일 로드"""
        ...
        return self._process(df_raw, 'csv_data', config)
    ...
    def _load_excel(
        ...
        if len(target_sheets) == 1:
            return self._load_single_sheet(file_obj, target_sheets[0], config)
        ...
        for idx, sheet in enumerate(target_sheets, 1):
            try:
                if progress_callback:
                    progress_callback(sheet, idx, total_sheets)
                
                df = self._load_single_sheet(file_obj, sheet, config)
                if df is not None and not df.empty:
                    results[sheet] = df
            except Exception as e:
                self.logger.error(f"Error processing sheet '{sheet}': {str(e)}")
                if config.get('error_handling', {}).get('on_parse_error') == 'raise':
                    raise
        
        return results
    
    def _load_single_sheet(self, file_obj, sheet_name: str, config: Dict) -> pd.DataFrame:
        """단일 시트 로드 및 처리"""
        ...
        return self._process(df_raw, sheet_name, config)
    
    def _process(self, df_raw: pd.DataFrame, source_name: str, config: Dict) -> pd.DataFrame:
        """데이터 처리 파이프라인"""
        ...
        # 5. 타임스탬프 처리
        df_data = self._process_timestamp(df_data, config)
        
        # 6. 컬럼명 정규화
        df_data = self._normalize_column_names(df_data, config)
        
        # 7. 데이터 타입 변환
        df_data = self._convert_types(df_data, config)
        
        # 8. 샘플링
        df_data = self._apply_sampling(df_data, config)
        
        # 9. 타임스탬프 제거 (옵션)
        df_data = self._remove_timestamp_if_needed(df_data, config)
        
        # 10. 후처리
        df_data = self._post_process(df_data, config)
    ...
    def _process_timestamp(self, df: pd.DataFrame, config: Dict) -> pd.DataFrame:
        """타임스탬프 처리"""
    ...
    def _apply_sampling(self, df: pd.DataFrame, config: Dict) -> pd.DataFrame:
        """샘플링 적용"""
    ...
    def _remove_timestamp_if_needed(self, df: pd.DataFrame, config: Dict) -> pd.DataFrame:
        """타임스탬프 제거 (옵션)"""
        ts_config = config.get('timestamp', {})
    ...
    def _normalize_column_names(self, df: pd.DataFrame, config: Dict) -> pd.DataFrame:
        """컬럼명 정규화"""
    ...
    def _post_process(self, df: pd.DataFrame, config: Dict) -> pd.DataFrame:
        """후처리"""
    ...
    def prepare_for_display(self, df: pd.DataFrame) -> pd.DataFrame:
        """DataFrame을 Streamlit 표시용으로 변환 (PyArrow 호환)"""
        import copy
        
        # attrs 백업
        original_attrs = copy.deepcopy(df.attrs) if hasattr(df, 'attrs') else {}
        
        # 새로운 DataFrame 생성 (완전히 새로 만들기)
        ...
        for col in df_safe.columns:
            try:
                ...
                # 2. object 타입 처리 (혼합 타입 위험)
                    ...
                    # Timestamp 객체가 있는지 확인
                    sample = non_null.head(10)
                    has_timestamp = any(isinstance(x, (pd.Timestamp, type(pd.NaT), datetime)) 
                                       for x in sample)
                    ...
                    if has_timestamp:
                        ...
                        # 일반 object는 string으로 통일
                        df_safe[col] = df_safe[col].astype(str)
                        df_safe[col] = df_safe[col].replace(['nan', 'None', 'NaT'], '')
                
                # 3. 숫자형 타입은 그대로 유지
                elif pd.api.types.is_numeric_dtype(df_safe[col]):
                    pass
                
                # 4. boolean 타입도 그대로 유지
                elif df_safe[col].dtype == 'bool':
                    pass
                
            except Exception as e:
                self.logger.warning(f"Column '{col}' conversion warning: {e}")
                # 실패 시 string으로 강제 변환
                try:
                    df_safe[col] = df_safe[col].astype(str)
                    df_safe[col] = df_safe[col].replace(['nan', 'None', 'NaT'], '')
